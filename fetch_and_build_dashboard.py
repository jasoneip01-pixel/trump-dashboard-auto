...
def fetch_github():
    ...
    for fname, url in FILES.items():
        ...
        if local.exists() and local.stat().st_size > 200:
            log.info("已缓存 %.1fKB %s", local.stat().st_size / 1024, fname)
            ok += 1
            continue
        try:
...
def fetch_market():
    ...
    for fname, ticker in TICKERS.items():
        ...
        try:
            ...
            r.raise_for_status()
            raw = r.json()
            ts = raw["chart"]["result"][0]["timestamp"]
            ...
            for i, t in enumerate(ts):
                ...
                records[d] = {
                    "open": round(float(open_[i] or close[i]), 4),
...
def wilson(c, n, z=1.96):
    ...
    d = 1 + z**2 / n
    ctr = (p + z**2 / (2 * n)) / d
    mg = z * math.sqrt(p * (1 - p) / n + z**2 / (4 * n**2)) / d
...
def spy_ret(date, hold, spy, dates):
    if date not in spy or date not in dates:
        return None
    try:
        i = dates.index(date)
        j = i + hold
...
def backtest():
    ...
    for rec in preds:
        ...
        if actual is not None:
            ...
            ok = bool(rec.get("correct", rec.get("is_correct", False)))
            signed = float(rec.get("market_return", rec.get("return", 0.0)))
        total += 1
        correct += int(ok)
        rets.append(signed)
        equity.append(equity[-1] * (1 + signed))

        m = by_model.setdefault(model, {"c": 0, "n": 0, "r": []})
        m["n"] += 1
        m["c"] += int(ok)
        m["r"].append(signed)

        ym = by_month.setdefault(date[:7], {"c": 0, "n": 0})
        ym["n"] += 1
        ym["c"] += int(ok)
    ...
    wr = correct / total
    clo, chi = wilson(correct, total)
    z = (wr - .5) / math.sqrt(.25 / total)
    ...
    for mid, d in by_model.items():
        ...
        mwr = d["c"] / d["n"]
        mci = wilson(d["c"], d["n"])
        mavg = mean(d["r"]) if d["r"] else 0
        models_out[mid] = {
            "win_rate": round(mwr, 4),
            "total": d["n"],
            "avg_return": round(mavg, 6),
            "ci_lo": mci[0],
            "ci_hi": mci[1]
        }
    ...
    for ym, d in sorted(by_month.items()):
        mwr = d["c"] / d["n"] if d["n"] else 0
        months_out[ym] = {"win_rate": round(mwr, 4), "total": d["n"]}
    ...
    result = {
        ...
        "ci_lo": clo,
        "ci_hi": chi,
        "ci_str": f"{clo:.1%}–{chi:.1%}",
        "z_score": round(z, 3),
        "significant": abs(z) > 1.96,
        "avg_return": round(avg, 6),
        "sharpe": round(sharpe, 3),
        "max_drawdown": round(mdd, 4),
        "cumulative_return": round(equity[-1] / equity[0] - 1, 4),
        "equity_curve": [round(v, 4) for v in equity],
        "models": models_out,
        "months": months_out,
        "spy_days": len(spy),
        "data_source": "real_spy" if spy else "log_only",
        "perm_p": perm_p(correct, total),
...
def build_html(bt):
    ...
    update_footer = f"""
    <div style="text-align:center; font-size:11px; color:#8aa4b8; margin:16px 0 8px; padding:8px; background:#0d1318; border-radius:4px; border:1px solid #1e2d3d;">
        <strong>Last Updated:</strong> {now_str} &nbsp;|&nbsp; 
        Win Rate: <span style="color:{'#00e5a0' if bt.get('win_rate',0)>=0.58 else '#f5a623' if bt.get('win_rate',0)>=0.53 else '#ff4d6a'}">{bt.get("win_rate_pct","N/A")}</span> &nbsp;|&nbsp; 
        Sharpe: <span style="color:{'#00e5a0' if bt.get('sharpe',0)>1 else '#f5a623'}">{bt.get("sharpe","?")}</span>
    </div>
    """
...
<div class="fl">数据: GitHub sstklen/trump-code + Yahoo Finance</div>
</footer>
